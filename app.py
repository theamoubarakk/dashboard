# app.py — One-Page EDA (no KPIs) with unified colors & report-matching titles
import os
import numpy as np
import pandas as pd
...
def load_suppliers():
    ...
    # Any quantity column for the "Total Product Quantity per Year" chart
    qty_col = None
...
# ---------- GRID: 3 × 2 (all charts height ~ 240px to fit one page) ----------
# Row 1
...
with r1c1:
    ...
    if sales_f is not None and not sales_f.empty and "Month" in sales_f.columns:
        ...
        fig.update_layout(height=240, margin=dict(l=6,r=6,t=6,b=6))
        st.plotly_chart(fig, use_container_width=True, key="m_sales_trend")
    else:
...
with r1c2:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Annual Supplier Order Amount by Category (2017–2024)</div>', unsafe_allow_html=True)
    if sup_f is not None and not sup_f.empty and sup_f["Year"].notna().any():
        ...
        # keep stable category colors
        fig = px.line(agg, x="Year", y="Order_Amount", color="Category",
                      markers=True, template="plotly_white",
                      color_discrete_map=CAT_COLORS)
        fig.update_traces(line=dict(width=2), marker=dict(size=5))
        fig.update_layout(height=240, margin=dict(l=6,r=6,t=6,b=6), legend_title_text="")
        st.plotly_chart(fig, use_container_width=True, key="sup_year_cat_line")
    else:
...
with r2c1:
    ...
    if sales_f is not None and not sales_f.empty:
        by_cat = (sales_f.groupby("Category", as_index=False)["Revenue"].sum()
                         .sort_values("Revenue", ascending=True))  # horizontal bar ascending
        fig = px.bar(by_cat, x="Revenue", y="Category", orientation="h",
                     template="plotly_white", color="Category",
                     color_discrete_map=CAT_COLORS)
        fig.update_layout(height=240, margin=dict(l=6,r=6,t=6,b=6), showlegend=False)
        st.plotly_chart(fig, use_container_width=True, key="rev_by_cat_hbar")
    else:
...
with r2c2:
    ...
    if sup_f is not None and not sup_f.empty:
        ...
        fig = px.bar(plot_df, x="ShopName", y="Order_Amount", color="Category",
                     ...
                     color_discrete_map=CAT_COLORS)
        fig.update_layout(height=240, margin=dict(l=6,r=6,t=6,b=6), legend_title_text="")
        st.plotly_chart(fig, use_container_width=True, key="top5_shops_stack")
    else:
...
with r3c1:
    ...
    if sales_f is not None and not sales_f.empty and {"Year","month_num"}.issubset(sales_f.columns):
        ...
        heat = (mat.pivot_table(index="Year", columns="month_num", values="Revenue", aggfunc="sum")
                   .fillna(0).sort_index())
        heat.columns = [str(m) for m in heat.columns]  # 1..12 as strings
        ...
        fig.update_layout(height=240, margin=dict(l=6,r=6,t=6,b=6))
        st.plotly_chart(fig, use_container_width=True, key="rev_heatmap")
    else:
...
with r3c2:
    ...
    if sup_f is not None and not sup_f.empty and qty_col is not None:
        ...
        fig.update_layout(height=240, margin=dict(l=6,r=6,t=6,b=6), showlegend=False)
        st.plotly_chart(fig, use_container_width=True, key="qty_per_year")
    else:
...
# ---------- FOOTER ----------
st.caption("One-page EDA — monthly trend & seasonality, category revenue, supplier trends & concentration. Colors unified for consistency with the report.")
